...
public class Baekjoon_backtracking_15665 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int fromIndex = 0; fromIndex < N; fromIndex++)
            for (int toIndex = 0; toIndex < N; toIndex++)
                if ( ascSortedNumbers.indexOf(ascSortedNumbers.get(toIndex)) == toIndex )
        ...
        for (int startIndex = 0; startIndex < N; startIndex++)
            if ( startIndex == 0 || ascSortedNumbers.get(startIndex) != ascSortedNumbers.get(startIndex-1) )
                dfs(startIndex);
