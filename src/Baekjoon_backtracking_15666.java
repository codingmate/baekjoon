...
public class Baekjoon_backtracking_15666 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int fromIndex = 0; fromIndex < N; fromIndex++)
            for (int toIndex = 0; toIndex < N; toIndex++)
                if ( ascSortedNumbers.get(fromIndex) <= ascSortedNumbers.get(toIndex)
                  && ascSortedNumbers.indexOf(ascSortedNumbers.get(toIndex)) == toIndex )
        ...
        for (int startIndex = 0; startIndex < N; startIndex++)
            if ( startIndex == ascSortedNumbers.indexOf(ascSortedNumbers.get(startIndex)) )
                dfs(startIndex);
    ...
    public static void dfs(int fromIndex) {
        ...
        for (int toIndex = 0; toIndex < N; toIndex++)
            if (printStack.size() < M
                    && graph[fromIndex][toIndex] == 1
                    && ascSortedNumbers.get(fromIndex) <= ascSortedNumbers.get(toIndex)) {
                dfs(toIndex);
            }
        if (printStack.size() == M) {
